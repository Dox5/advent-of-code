...
decodeColour _ = error "colour must be 0 or 1"

decodeDirection :: Int -> Direction
decodeDirection 0 = Lib.Left
decodeDirection 1 = Lib.Right
decodeDirection _ = error "direction must be 0 or 1"

runBot :: State Glue ()
runBot = do
  (r, b@(s, vm)) <- State.get
  case s of
    IntCode.Halted -> return ()

    IntCode.InputRequested -> do
      let
        c  = State.evalState detectColour r
        b' = IntCode.resumeVMWithInput (encodeColour c) b
      State.put (r, b')
      runBot
    ...
    (IntCode.Output p) -> do
      let
        c = decodeColour p
        b'@((IntCode.Output d), _) = IntCode.resumeVM b
        t = decodeDirection d
        r' = State.execState (paint c >> turn t >> move) r
      State.put (r', IntCode.resumeVM b')
      runBot


getGridSize :: State Glue Int
getGridSize = State.gets (\(r, _) -> HashMap.size $ grid r)

renderBot :: State Glue String
renderBot = do
  (r, _) <- State.get
  return $ State.evalState (render) r

main :: IO ()
main = do
  let sz_blk = State.evalState (runBot >> getGridSize) (bootRobot, brain)
  putStrLn . showString "Number of tiles painted at least once: " . show $ sz_blk
  putStrLn "Rendered code:"

  let img = State.evalState (runBot >> renderBot) (bootRobotWithPainted White, brain)
  putStrLn img
