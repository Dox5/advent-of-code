...
int main()
{
  ...
  std::for_each(intersection.begin(), intersection.end(), [&first, &second, &distances](coordinates const & c)
  {
    // Find in each of first, second
    auto a = first.find(c);
    auto b = second.find(c);

    auto distance = a->steps + b->steps;
